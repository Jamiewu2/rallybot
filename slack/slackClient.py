...
class RallyBotClient:
    # constants
    RTM_READ_DELAY = 1  # 1 second delay between reading from RTM
    EXAMPLE_COMMAND = "do"
    MENTION_REGEX = "^<@(|[WU].+?)>(.*)"

    def __init__(self, slack_client, rally_client):
        self.slack_client = slack_client
        self.rally_client = rally_client
    ...
    def run(self):
        if self.slack_client.rtm_connect(with_team_state=False):
            ...
            while True:
                ...
                time.sleep(RallyBotClient.RTM_READ_DELAY)
        else:
    ...
    def parse_bot_commands(self, slack_events):
        ...
        for event in slack_events:
            if event["type"] == "message" and not "subtype" in event:
                user_id, message = RallyBotClient.parse_direct_mention(event["text"])
    ...
    def parse_direct_mention(message_text):
        ...
        """
        matches = re.search(RallyBotClient.MENTION_REGEX, message_text)
